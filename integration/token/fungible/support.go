...
const (
	...
	eventualCheckPolling = 500 * time.Millisecond
)
...
func CheckPublicParamsForTMSID(network *integration.Infrastructure, tmsId *token2.TMSID, ids ...*token3.NodeReference) {
	for _, id := range ids {
		for _, replicaName := range id.AllNames() {
			...
			gomega.Eventually(func(g gomega.Gomega, replicaName string, id *token3.NodeReference) {
			...
			}).WithArguments(replicaName, id).WithTimeout(eventualCheckTimeout).WithPolling(eventualCheckPolling).Should(gomega.Succeed())
		}
	}
}
